...
| S-02 | Debug `QualityTestReceiver` was exported without permission. | Another app could request telemetry or deliberately crash a debug installation. | High | MASVS-PLATFORM | Fixed: receiver is debug-only and guarded by the privileged `android.permission.DUMP`, so ADB can invoke it but ordinary apps cannot. |
