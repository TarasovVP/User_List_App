...
class UserListFilteringTest {
    ...
    @Test fun `search is case insensitive across name email and company`() {
    ...
    @Test fun `search does not use fields outside the product contract`() {
        assertTrue(search("engineer").isEmpty())
        assertTrue(search("user").isEmpty())
    }

    @Test fun `sorting supports ascending and descending full name order`() {
        val users = listOf(grace, ada)

        assertEquals(
            listOf(1, 2),
            filterAndSortUsers(users, "", UserSort.NAME_ASCENDING, favoritesOnly = false).map { it.id },
        )
        assertEquals(
            listOf(2, 1),
            filterAndSortUsers(users, "", UserSort.NAME_DESCENDING, favoritesOnly = false).map { it.id },
        )
    }

    @Test fun `favorites filter combines with search and sorting`() {
